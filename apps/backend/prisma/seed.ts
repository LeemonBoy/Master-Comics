...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'admin@mastercomics.local',
      username: 'admin',
      passwordHash,
  ...
  const creator = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'creator@mastercomics.local',
      username: 'creator',
      passwordHash,
      displayName: 'Creador',
      role: 'USER',
  ...
  const reader = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'reader@mastercomics.local',
      username: 'reader',
      passwordHash,
      displayName: 'Lector',
      role: 'USER',
  ...
  await prisma.comment.create({
    data: { userId: reader.id, comicId: comic.id, content: '¡Excelente cómic! Me encanta la ambientación.' },
  });
